...
class SamsungTVConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def _try_connect(self):
        """Try to connect to the TV and check auth."""
        for method in SUPPORTED_METHODS:
            ...
            if result != RESULT_CANNOT_CONNECT:
                return result
        LOGGER.debug("No working connection method found for %s", self._host)
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if self._bridge.method == METHOD_LEGACY:
                self.discovery_infos = await ssdp.async_get_discovery_info_by_st(self.hass, "urn:samsung.com:device:RemoteControlReceiver:1")
