"""INIT for the Legacy Samsung TV integration."""
...
LEGACY_SAMSUNG_PLATFORM = [Platform.MEDIA_PLAYER]

SamsungTVConfigEntry = ConfigEntry[SamsungTVDataUpdateCoordinator]

def ensure_unique_hosts(value):
    """Validate that all configs have a unique host (IPv4 check only)."""
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    if DOMAIN in config:
        ...
        for entry_config in config[DOMAIN]:
            ...
            if valid_entries:
                LOGGER.debug("Ignoring configuration for %s, already has a configuration", ip_address)
    ...
    if not mac or mac_is_incorrectly_formatted:
        ...
        if not mac:
            mac = await hass.async_add_executor_job(
                partial(get_mac_address, ip=host, network_request=True)
           )
